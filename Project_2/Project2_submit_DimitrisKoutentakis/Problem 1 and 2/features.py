...
def projectOntoPC(X, pcs, n_components):
    ...
    # print(Xcenter, pcs)
    projection = np.dot(Xcenter,pcs[:,:n_components])
    return projection
    ...
    for i in range(n):
        ...
        for j in range(d+1):
            ...
            for k in range(j+1,d+1):
                d2=X_withones[i,k]
                # print('d2 = ', d2)
                feature+=[3**.5*d1**2*d2,3**.5*d2**2*d1]
                for z in range(k+1,d+1):
                    d3=X_withones[i,z]
                    feature+=[6**.5*d1*d2*d3]
        newData[i]=np.array(feature)
